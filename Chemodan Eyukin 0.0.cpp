...
int main()
{
    ...
    // Заголовок программы
    SetConsoleTextAttribute(h, 0x0E); // жёлтый
    cout << "Программа для подсчёта количества целых чисел\n";
    cout << "в диапазоне от 100 до 999,\n";
    cout << "у которых есть две или три одинаковые цифры\n\n";
    SetConsoleTextAttribute(h, 0x07); // стандартный цвет
    ...
    for (int a = 1; a <= 9; a++)
    {
        for (int b = 0; b <= 9; b++)
        {
            for (int c = 0; c <= 9; c++)
            {
                ...
                // Условие: есть хотя бы две одинаковые цифры
                bool ok = (a == b) || (a == c) || (b == c);
                // Тернарные операторы
                count += ok ? 1 : 0;
                ok && (
